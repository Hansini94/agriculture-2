...
Class Management_support extends CI_Controller {
    ...
    private $table_name = "tbl_experimental_non_academic_staff";
    private $table_name1 = "tbl_experimental_non_academic_staff_caption";
    private $table_name2 = "tbl_experimental_non_academic_staff_category";
    private $page_id = "11";
    ...
    public function index() {
        ...
        $this->load->view('adminpanel/staff/management_support_category_view', $data);
    ...
    public function save_category($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name2)) {
                ...
                redirect(base_url() . 'adminpanel/staff/management_support');
            } else {
    ...
    public function management_support_staffs() {
        ...
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/staff/management_support_staff_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/staff/management_support/management_support_staffs/'.$iCatId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/staff/management_support/management_support_staffs/'.$iCatId);
            }
    ...
    public function change_status() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_edit')) {
            $this->common_library->flexigrid_change_status('adminpanel/staff/management_support/management_support_staffs/'.$iCatId, $this->table_name);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/staff/management_support/management_support_staffs/'.$iCatId);
        }
    ...
    public function delete_record() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_delete')) {
            $this->common_library->flexigrid_delete_record('adminpanel/staff/management_support/management_support_staffs/'.$iCatId, $this->table_name);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/staff/management_support/management_support_staffs/'.$iCatId);
        }
    ...
    public function add_caption() {        
        ...
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/staff/management_support_caption_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
    ...
    public function save_caption($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name1)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/staff/management_support/add_caption');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/staff/management_support/add_caption');
            }
