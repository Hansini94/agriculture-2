...
class Admin_model extends CI_Model {
    ...
    public function form_names() {
        //return $query = $this->db->get('tbl_abc_dyn_menu');
		$this->db->where('tbl_abc_dyn_menu.show_menu', 1);
        $this->db->order_by("fOrder", "ASC");
        $result = $this->db->get('tbl_abc_dyn_menu');
    ...
    public function get_user_types() {
        ...
        $result = $this->db->get('tbl_abc_user_type');
    ...
    public function get_user_detail() {
        ...
        $this->db->select('tbl_abc_backend_user.id,tbl_abc_backend_user.vFirstName,tbl_abc_backend_user.vUserName,tbl_abc_backend_user.vLastName,tbl_abc_backend_user.cEnable,tbl_abc_backend_user.vContactNo,tbl_abc_backend_user.vEmail,tbl_abc_backend_user.iUserType');
        $this->db->from('tbl_abc_backend_user');
        $this->db->join('tbl_abc_user_type', 'tbl_abc_backend_user.iUserType = tbl_abc_user_type.id');
        $this->db->order_by("tbl_abc_backend_user.vFirstName", "ASC");
        $result = $this->db->get();
    ...
    public function save_user_type($save_status) {
        ...
        if ($save_status === 'A') {
            $this->db->trans_start();
            $this->db->insert('tbl_abc_user_type', $data);
            ...
            for ($i = 0; $i < count($rowCount); $i++) {
                ...
                $query = $this->db->insert('tbl_abc_privilage', $data_t);
            }
            ...
            $this->db->update('tbl_abc_user_type', $data);
            $this->db->where('iUserTypeID', $user_type_id);
            $this->db->delete('tbl_abc_privilage');
    ...
    public function privilage_form_names($user_type_id) {
        if ($user_type_id != '') {
            ...
            $this->db->where('iUserTypeID', $user_type_id);
            $res = $this->db->get('tbl_abc_privilage');
            if ($res->num_rows() > 0) {
    ...
    public function delete_privilages() {
        ...
        $this->db->delete('tbl_abc_user_type');
