...
Class Visiting_professors extends CI_Controller {
    ...
    private $table_name = "tbl_food_visiting_professor";
    private $page_id = "4";
    ...
    public function save_visiting_professor($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_food_visiting_professor')) {
            ...
            if ($this->common_model->save_data('tbl_food_visiting_professor')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/staff/visiting_professors');
