...
Class Innerpage_banner extends CI_Controller {
    ...
    private $table_name = "tbl_animal_innerpage_banner";
    private $page_id = "32";
...
// 		$data['edit_main_slider'] = $this->home_model->get_edit_main_slider($sliderid);
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_animal_innerpage_banner')) {
            ...
            if ($this->common_model->save_data('tbl_animal_innerpage_banner')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/home_page/innerpage_banner');
