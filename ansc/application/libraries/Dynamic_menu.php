...
Class Dynamic_menu {
    ...
    function build_menu($iUserType) {
        ...
        if ($this->ci->session->userdata('is_logged_inbackendsession') != "1") {
            ...
            $query_string = "SELECT
tbl_animal_dyn_menu.id,
tbl_animal_dyn_menu.icon,
tbl_animal_dyn_menu.title,
tbl_animal_dyn_menu.page_id,
tbl_animal_dyn_menu.url,
tbl_animal_dyn_menu.parent_id,
tbl_animal_dyn_menu.is_parent,
tbl_animal_dyn_menu.show_menu,
tbl_animal_dyn_menu.parent_order,
tbl_animal_dyn_menu.child_order

FROM
tbl_animal_privilage
INNER JOIN tbl_animal_dyn_menu ON tbl_animal_privilage.iFormID = tbl_animal_dyn_menu.id
WHERE
tbl_animal_privilage.iUserTypeID = $iUserType AND tbl_animal_dyn_menu.show_menu = 1
AND tbl_animal_privilage.vPrivilages != '0,0,0'
ORDER BY
tbl_animal_dyn_menu.parent_order ASC";
...
tbl_animal_dyn_menu.parent_id = $id AND tbl_animal_dyn_menu.show_menu = 1
AND tbl_animal_privilage.vPrivilages != '0,0,0'
ORDER BY
tbl_animal_dyn_menu.child_order ASC";
        $query = $this->ci->db->query($query_string);
