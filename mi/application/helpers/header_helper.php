...
if (!function_exists('get_meta')) {
    ...
    function get_meta($page) {
        ...
        $ci = & get_instance();
        $ci->load->database();
        $sql = " SELECT * FROM tbl_mi_meta_tags where vPage_class='$page' ";
        $q = $ci->db->query($sql);
...
if (!function_exists('get_contact')) {
    ...
    function get_contact() {
        ...
        $ci = & get_instance();
        $ci->load->database();
        $sql = " SELECT * FROM tbl_mi_contact_us where id='1' ";
        $q = $ci->db->query($sql);
        return $q->result();
    }
...
if (!function_exists('get_faq_list')) {
    function get_faq_list($faq_id) {
        ...
        $ci->db->from('tbl_mi_faq');
        $ci->db->where('iFaqId', $faq_id);
        $ci->db->where('tbl_mi_faq.cEnable', 'Y');
        $ci->db->order_by('tbl_mi_faq.iOrder', 'asc');
        $result = $ci->db->get();
        //echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_quality_downloads')) {
    function get_quality_downloads() {
        ...
        $ci->db->from('tbl_mi_quality_download');
        $ci->db->where('cEnable', 'Y');
        $ci->db->order_by('iOrder', 'asc');
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_mobile_sliders')) {
    function get_mobile_sliders() {
        ...
        $ci->db->from('tbl_mi_mobile_slider');
        $ci->db->where('cEnable', 'Y');
        $ci->db->order_by('iOrder', 'asc');
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_innerpage_banner')) {
    ...
    function get_innerpage_banner($pageRoute) {
        ...
        $ci->db->from('tbl_mi_innerpage_banner');
