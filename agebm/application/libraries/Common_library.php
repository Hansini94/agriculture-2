...
Class Common_library {
    ...
    public function flexigrid_change_status($redirect_path, $tbl_aebm_name) {

        $userID = $this->ci->security->xss_clean($this->ci->uri->segment(6));
       
        $this->ci->common_model->chge_status($userID, $tbl_aebm_name);
	...
	public function flexigrid_update_record($tbl_aebm_name) {

        $userID = $this->ci->security->xss_clean($this->ci->uri->segment(5));
        $data = array();

        $data = $this->ci->common_model->get_edit_data($userID, $tbl_aebm_name);
        $data['saveStatus'] = 'E';

        return $data;
    }

    public function flexigrid_update_user($tbl_aebm_name) {
    ...
    public function flexigrid_delete_record($redirect_path, $tbl_aebm_name) {
        $userID = $this->ci->security->xss_clean($this->ci->uri->segment(6));
//echo 'dd111111111'; exit();
        $this->ci->common_model->del_records($userID, $tbl_aebm_name);
