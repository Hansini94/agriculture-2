...
if (!function_exists('get_meta')) {
    ...
    function get_meta($page) {
        ...
        $ci = & get_instance();
        $ci->load->database();
        $sql = " SELECT * FROM tbl_aebm_meta_tags where vPage_class='$page' ";
        $q = $ci->db->query($sql);
...
if (!function_exists('get_contact')) {
    ...
    function get_contact() {
        ...
        $ci = & get_instance();
        $ci->load->database();
        $sql = " SELECT * FROM tbl_aebm_contact_us where id='1' ";
        $q = $ci->db->query($sql);
        return $q->result();
    }
...
if (!function_exists('get_faq_list')) {
    function get_faq_list($faq_id) {
        ...
        $ci->db->from('tbl_aebm_faq');
        $ci->db->where('iFaqId', $faq_id);
        $ci->db->where('tbl_aebm_faq.cEnable', 'Y');
        $ci->db->order_by('tbl_aebm_faq.iOrder', 'asc');
        $result = $ci->db->get();
        //echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_quality_downloads')) {
    function get_quality_downloads() {
        ...
        $ci->db->from('tbl_aebm_quality_download');
        $ci->db->where('cEnable', 'Y');
        $ci->db->order_by('iOrder', 'asc');
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_main_sliders')) {
    function get_main_sliders() {
        ...
        $ci->db->from('tbl_aebm_main_slider');
        $ci->db->where('cEnable', 'Y');
        $ci->db->order_by('iOrder', 'asc');
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_mobile_sliders')) {
    function get_mobile_sliders() {
        ...
        $ci->db->from('tbl_aebm_mobile_slider');
        $ci->db->where('cEnable', 'Y');
        $ci->db->order_by('iOrder', 'asc');
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
...
if (!function_exists('get_contact_details')) {
    function get_contact_details() {
        ...
        $ci->db->from('tbl_aebm_contact_us');
        $ci->db->where('id', 1);
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
        if ($result->num_rows() > 0) {
...
if (!function_exists('get_logo')) {
    function get_logo() {
        ...
        $ci->db->from('tbl_aebm_home_detail');
        $ci->db->where('id', 1);
        $result = $ci->db->get();
        // echo $ci->db->last_query();  exit();  
        if ($result->num_rows() > 0) {
...
if (!function_exists('get_innerpage_banner')) {
    ...
    function get_innerpage_banner($pageRoute) {
        ...
        $ci->db->from('tbl_aebm_innerpage_banner');
