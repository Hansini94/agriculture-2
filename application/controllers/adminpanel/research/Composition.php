...
Class Composition extends CI_Controller {
    ...
    private $table_name = "tbl_composition";
    private $page_id = "25";
    private $redirect_path = "adminpanel/research/composition";
    ...
    public function index() {
        ...
        $data['cSaveStatus']= 'A';
        $data['list_data'] = $this->research_model->get_composition_list();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/research/composition_view', $data);
        $this->load->view('adminpanel/footer_view');
    }

    public function save_composition($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_composition')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/research/composition');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/research/composition');
            }
        } else {
            if ($this->common_model->save_data('tbl_composition')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/research/composition');
	...
	public function edit_composition() {
		...
		$data['edit_composition'] = $this->research_model->get_edit_composition($compositionId);
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/research/composition_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
