...
Class Bsc_agtech_introduction extends CI_Controller {

    private $table_name = "tbl_bsc_agtech_mgt";
    private $page_id = "82";
    private $redirect_path = "adminpanel/academics/bsc_agtech_introduction";
    ...
    public function index() {	
        ...
        $this->load->view('adminpanel/academics/bsc_agtech_introduction_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
	

    public function save_bsc_agtech($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_bsc_agtech_mgt')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_introduction');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_introduction');
            }
        } else {
            if ($this->common_model->save_data('tbl_bsc_agtech_mgt')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_introduction');
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/academics/bsc_agtech_introduction/update_details/".$imageID);
            } else {
