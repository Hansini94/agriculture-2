...
Class Bsc_agtech_requirement extends CI_Controller {

    private $table_name = "tbl_bsc_requirement";
    private $page_id = "97";
    private $redirect_path = "adminpanel/academics/bsc_agtech_requirement";
    ...
    public function index() {	
        ...
        $data['requirement_data'] = $this->academic_model->get_bsc_agtech_requirement_data();
        // var_dump($data['undergraduate_data']);exit();
		//echo 'ff'; exit();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/academics/bsc_agtech_requirement_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
	

    public function save_bsc_requirement($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_bsc_requirement')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_requirement');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_requirement');
            }
        } else {
            if ($this->common_model->save_data('tbl_bsc_requirement')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_agtech_requirement');
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/academics/bsc_agtech_requirement/update_details/".$imageID);
            } else {
