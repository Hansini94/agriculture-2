...
Class Bsc_asf_introduction extends CI_Controller {

    private $table_name = "tbl_bsc_asf_introduction";
    private $page_id = "87";
    private $redirect_path = "adminpanel/academics/bsc_asf_introduction";
    ...
    public function index() {	
        ...
        $data['ckeditor_tOutlineSpeNotice'] = array(
            //ID of the textarea that will be replaced
            'id' => 'tOutlineSpeNotice',
            'path' => 'assets/js/ckeditor',
            //Optionnal values
            'config' => array(
                'toolbar' => "Full", //Using the Full toolbar
        ...
        $data['introduction_data'] = $this->academic_model->get_bsc_asf_introduction_data();
		//echo 'ff'; exit();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/academics/bsc_asf_introduction_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
	

    public function save_bsc_asf($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_bsc_asf_introduction')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_asf_introduction');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/academics/bsc_asf_introduction');
            }
        } else {
            if ($this->common_model->save_data('tbl_bsc_asf_introduction')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academics/bsc_asf_introduction');
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/academics/bsc_asf_introduction/update_details/".$imageID);
            } else {
