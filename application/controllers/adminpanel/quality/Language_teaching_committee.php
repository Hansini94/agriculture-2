...
Class Language_teaching_committee extends CI_Controller {

    private $table_name = "tbl_language_teaching_committee";
    private $table_name2 = "tbl_teaching_committee_composition";
    private $page_id = "60";
    private $redirect_path = "adminpanel/quality/language_teaching_committee";
    ...
    public function index() {	
        ...
        $this->load->view('adminpanel/quality/language_teaching_committee_view', $data);
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name)) {
                ...
                redirect(base_url() . 'adminpanel/quality/language_teaching_committee');
            } else {
    ...
    public function composition(){
        ...
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/quality/teaching_committee_composition_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
    ...
    public function save_data_composition($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name2)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId);
            }
    ...
    public function change_status_member() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_edit')) {
            $this->common_library->flexigrid_change_status('adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId);
        }
    ...
    public function delete_record_composition() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_delete')) {
            $this->common_library->flexigrid_delete_record('adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/language_teaching_committee/composition/'.$iCommitteeId);
        }
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/quality/language_teaching_committee/edit/".$imageID);
            } else {
