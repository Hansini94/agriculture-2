...
Class Time_table_committee_composition extends CI_Controller {

    private $table_name = "tbl_time_table_committee_period";
    private $table_name2 = "tbl_time_table_committee_composition";
    private $page_id = "65";
    private $redirect_path = "adminpanel/quality/time_table_committee_composition";
    ...
    public function index() {
        ...
        $this->load->view('adminpanel/quality/time_table_committee_period_view', $data);
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name)) {
                ...
                redirect(base_url() . 'adminpanel/quality/time_table_committee_composition');
            } else {
    ...
    public function save_data_composition($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name2)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId);
            }
    ...
    public function change_status_composition() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_edit')) {
            $this->common_library->flexigrid_change_status('adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId);
        }
    ...
    public function delete_record_composition() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_delete')) {
            $this->common_library->flexigrid_delete_record('adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/time_table_committee_composition/add_composition/'.$iPeriodId);
        }
