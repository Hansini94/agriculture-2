...
Class Curriculum_development_committee extends CI_Controller {
    ...
    public function save_data_composition($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name2)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId);
            }
    ...
    public function change_status_member() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_edit')) {
            $this->common_library->flexigrid_change_status('adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId);
        }
    ...
    public function delete_record_composition() {
        ...
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_delete')) {
            $this->common_library->flexigrid_delete_record('adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() . 'adminpanel/quality/curriculum_development_committee/composition/'.$iCommitteeId);
        }
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/quality/curriculum_development_committee/edit/".$imageID);
            } else {
