...
Class Home extends CI_Controller {
    ...
    private $table_name = "tbl_faculty_staff_content";
    private $page_id = "279";
    private $redirect_path = "adminpanel/faculty_staff/home";
    ...
    public function index() {	
        ...
        $data['edit_date'] = $this->faculty_staff_model->get_faculty_staff_data();
		//echo 'ff'; exit();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/faculty_staff/home_view', $data);
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_faculty_staff_content')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/faculty_staff/home');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/faculty_staff/home');
            }
        } else {
            if ($this->common_model->save_data('tbl_faculty_staff_content')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/faculty_staff/home');
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/faculty_staff/home/update_details/".$imageID);
            } else {
