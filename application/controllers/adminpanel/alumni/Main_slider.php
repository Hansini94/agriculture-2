...
Class Main_slider extends CI_Controller {
    ...
    private $table_name = "tbl_alumni_main_slider";
    private $page_id = "259";
    private $redirect_path = "adminpanel/alumni/main_slider";
    ...
    public function index() {
        $data['cSaveStatus']= 'A';
        $data['list_data'] = $this->alumni_model->get_main_slider_list();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/alumni/main_slider_view', $data);
	...
	public function edit_main_slider() {
		...
		$data['edit_main_slider'] = $this->alumni_model->get_edit_main_slider($sliderid);
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/alumni/main_slider_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
    ...
    public function save_main_slider($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_alumni_main_slider')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/alumni/main_slider');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/alumni/main_slider');
            }
        } else {
            if ($this->common_model->save_data('tbl_alumni_main_slider')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/alumni/main_slider');
