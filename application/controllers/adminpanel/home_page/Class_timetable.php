...
Class Class_timetable extends CI_Controller {
    ...
    private $table_name = "tbl_class_timetable";
    private $table_name2 = "tbl_class_timetable_list";
    private $page_id = "75";
    private $redirect_path = "adminpanel/home_page/class_timetable";
    private $redirect_path2 = "adminpanel/home_page/class_timetable/add_download";
    ...
    public function index() {
        ...
        $this->load->view('adminpanel/home_page/class_timetable_view', $data);
    ...
    public function save_class($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name)) {
                ...
                redirect(base_url() . 'adminpanel/home_page/class_timetable');
            } else {
    ...
    public function add_download() {
        $data['cSaveStatus']= 'A';
        $recID = $this->uri->segment(5);
        $data['list_data'] = $this->home_model->get_download_list($recID, $this->table_name2);
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/home_page/class_timetable_download_view', $data);
        $this->load->view('adminpanel/footer_view');
    }

    public function save_download($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $icttId = $this->input->post('icttId', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name2)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/home_page/class_timetable/add_download/'.$icttId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/home_page/class_timetable/add_download/'.$icttId);
            }
    ...
    public function change_status_download() {
        $recID = $this->uri->segment(7);
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_edit')) {
            $this->common_library->flexigrid_change_status('adminpanel/home_page/class_timetable/add_download/'.$recID, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() .'adminpanel/home_page/class_timetable/add_download/'.$recID);
        }
    }

    public function delete_record_download() {
        $recID = $this->uri->segment(7);
        $this->common_library->check_privilege('p_edit');
        if ($this->common_library->check_privilege('p_delete')) {
            $this->common_library->flexigrid_delete_record('adminpanel/home_page/class_timetable/add_download/'.$recID, $this->table_name2);
        } else {
            $this->session->set_flashdata('message_restricted', 'You do not have permission.');
            redirect(base_url() .'adminpanel/home_page/class_timetable/add_download/'.$recID);
        }
