...
Class Outreach_centers extends CI_Controller {

    private $table_name = "tbl_outreach_centers";
    private $page_id = "292";
    private $redirect_path = "adminpanel/services/outreach_centers";
    ...
    public function index() {	
        ...
        $data['faurs_page_date'] = $this->services_model->get_ocenters_data();
		//echo 'ff'; exit();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/services/outreach_centers_view', $data);
    ...
    public function save_data($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_outreach_centers')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/services/outreach_centers');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/services/outreach_centers');
            }
        } else {
            if ($this->common_model->save_data('tbl_outreach_centers')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/services/outreach_centers');
    ...
    public function remove_image() {
        if ($this->common_library->check_privilege('p_edit')) {
            ...
            if ($postimage_delete == TRUE) {
                redirect(base_url() . "adminpanel/services/outreach_centers/update_details/".$imageID);
            } else {
