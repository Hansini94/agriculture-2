...
Class Meeting_Schedule extends CI_Controller {
    ...
    private $table_name = "tbl_meeting_schedule";
    private $page_id = "15";
    private $redirect_path = "adminpanel/about_us/meeting_schedule";
    ...
    public function index() {
        ...
        $this->load->view('adminpanel/aboutus/meeting_schedule_view', $data);
        $this->load->view('adminpanel/footer_view');
    }

    public function save_meeting_schedules($data = '') {
        $cSaveStatus = $this->input->post('cSaveStatus', TRUE);
        $id = $this->input->post('id', TRUE);
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_meeting_schedule')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/about_us/meeting_schedule');
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/about_us/meeting_schedule');
            }
        } else {
            if ($this->common_model->save_data('tbl_meeting_schedule')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/about_us/meeting_schedule');
	...
	public function edit_meeting_schedule() {
		...
		$data['edit_meeting_schedule'] = $this->aboutus_model->get_edit_meeting_schedule($meetingId);
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/aboutus/meeting_schedule_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
