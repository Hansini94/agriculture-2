...
class Faculty_n_staff_model extends CI_Model {
    ...
    public function get_emeritus_professor() {
        $this->db->from('tbl_emeritus_professor');
    ...
    public function get_con_guidelines_content() {
        $this->db->from('tbl_conduct_guidelines');
        $this->db->where('id', 1);
        $result = $this->db->get();
        //echo $this->db->last_query();  exit();  
        if ($result->num_rows() > 0) {
    ...
    public function get_academic_staff() {
        $this->db->from('tbl_academic_staff_link');
        $this->db->where('cEnable', 'Y');
        $this->db->order_by('iOrder', 'asc');
        $result = $this->db->get();
        //echo $this->db->last_query();  exit();  
    ...
    public function get_declaration_form() {
        $this->db->from('tbl_declaration_form');
        $this->db->where('id', 1);
        $result = $this->db->get();
        //echo $this->db->last_query();  exit();  
        if ($result->num_rows() > 0) {
