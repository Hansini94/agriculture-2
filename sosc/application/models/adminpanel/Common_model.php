...
class Common_model extends CI_Model {
    ...
    public function add_log($tDes, $title = '') {
        ...
        $logData = array(
         ...
         $this->db->insert('tbl_soil_logs', $logData);
    ...
    public function save_data($tbl_soil_name) {
		$fields = $this->db->list_fields($tbl_soil_name);
        ...
        $query = $this->db->insert($tbl_soil_name, $data);
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function update_saved_data($tbl_soil_name) {
        $fields = $this->db->list_fields($tbl_soil_name);
        ...
        $query = $this->db->update($tbl_soil_name, $data, "id = $id");
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function update_password_data_useless($tbl_soil_name) {
        ...
        if ($oldpw == $olpdbpw) {
            ...
            $query = $this->db->update($tbl_soil_name, $data, "id = $uUserID");
            if ($query) {
                return true;
            } else {
                return false;
    ...
    public function update_password_data($tbl_soil_name) {
    ...
    function get_edit_data($userID, $tbl_soil_name) {
        $result = $this->db->get_where($tbl_soil_name, array('id' => $userID));
    ...
    function chge_status($userID, $tbl_soil_name) {
		//echo "hh";exit();
        $this->db->select('cEnable');
        $this->db->where('id', $userID);
        $query = $this->db->get($tbl_soil_name);
        ...
        $this->db->update($tbl_soil_name, $arrData, "id = $userID");
		//echo $this->db->last_query(); exit();
    }

    function del_records($userID, $tbl_soil_name) {
        $this->db->where('id', $userID);
        $this->db->delete($tbl_soil_name);
    }

    function get_sector_detail($sec_id) {
        $this->db->select('vCompanysector');
        $this->db->where('id', $sec_id);
        $query = $this->db->get('tbl_soil_company_sector');
    ...
    function get_page_detail($page_id) {
        if ($this->session->userdata('is_logged_inbackendsession') != "1") {
            ...
            $query_string = "SELECT
            tbl_soil_privilage.vPrivilages 
            FROM
            tbl_soil_privilage
    ...
    function get_last_serial($tbl_soil_name, $field_name) {
        $query = "select TOP 1 $field_name from $tbl_soil_name order by id DESC";
