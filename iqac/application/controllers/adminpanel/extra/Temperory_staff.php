...
Class Temperory_staff extends CI_Controller {
    ...
    private $table_name = "tbl_iqac_temperory_staff";
    private $page_id = "7";
    ...
    public function save_temperory_staff($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_iqac_temperory_staff')) {
            ...
            if ($this->common_model->save_data('tbl_iqac_temperory_staff')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/staff/temperory_staff');
