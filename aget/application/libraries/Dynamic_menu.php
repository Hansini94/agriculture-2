...
Class Dynamic_menu {
    ...
    function build_menu($iUserType) {
        ...
        if ($this->ci->session->userdata('is_logged_inbackendsession') != "1") {
            ...
            $query_string = "SELECT
tbl_aext_dyn_menu.id,
tbl_aext_dyn_menu.icon,
tbl_aext_dyn_menu.title,
tbl_aext_dyn_menu.page_id,
tbl_aext_dyn_menu.url,
tbl_aext_dyn_menu.parent_id,
tbl_aext_dyn_menu.is_parent,
tbl_aext_dyn_menu.show_menu,
tbl_aext_dyn_menu.parent_order,
tbl_aext_dyn_menu.child_order

FROM
tbl_aext_privilage
INNER JOIN tbl_aext_dyn_menu ON tbl_aext_privilage.iFormID = tbl_aext_dyn_menu.id
WHERE
tbl_aext_privilage.iUserTypeID = $iUserType AND tbl_aext_dyn_menu.show_menu = 1
AND tbl_aext_privilage.vPrivilages != '0,0,0'
ORDER BY
tbl_aext_dyn_menu.parent_order ASC";
...
tbl_aext_dyn_menu.parent_id = $id AND tbl_aext_dyn_menu.show_menu = 1
AND tbl_aext_privilage.vPrivilages != '0,0,0'
ORDER BY
tbl_aext_dyn_menu.child_order ASC";
        $query = $this->ci->db->query($query_string);
