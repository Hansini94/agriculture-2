...
class Ajax_flxi_model extends CI_Model {
    ...
    public function get_grid_data_join_reporteduser($valid_fields) {
        ...
        $this->db->from('tbl_aeu_report_user');
        $this->db->join('tbl_aeu_frontend_user', 'tbl_aeu_frontend_user.id = tbl_aeu_report_user.iUserid');
        ...
        $this->db->select('count(tbl_aeu_report_user.id) as record_count');
        $this->db->from('tbl_aeu_report_user');
        $this->db->join('tbl_aeu_frontend_user', 'tbl_aeu_frontend_user.id = tbl_aeu_report_user.iUserid');
