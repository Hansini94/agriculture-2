...
Class Main_slider extends CI_Controller {
    ...
    private $table_name = "tbl_livestock_main_slider";
    private $page_id = "2";
    ...
    public function save_main_slider($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data('tbl_livestock_main_slider')) {
            ...
            if ($this->common_model->save_data('tbl_livestock_main_slider')) {
                //$tDes = "saved data has been updated";
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/home_page/main_slider');
