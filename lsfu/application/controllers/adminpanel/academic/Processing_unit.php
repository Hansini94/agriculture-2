...
Class Processing_unit extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('adminpanel/academic/processing_unit_view', $data);
    ...
    public function save_series($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name)) {
                ...
                redirect(base_url() . 'adminpanel/academic/processing_unit');
            } else {
    ...
    public function gallery() {
        ...
        // var_dump("sdasd");exit();
        $this->load->view('adminpanel/header_view');
        $this->load->view('adminpanel/academic/processing_unit_gallery_view', $data);
        $this->load->view('adminpanel/footer_view');
    }
    ...
    public function save_gallery($data = '') {
        ...
        if ($cSaveStatus === 'E') {
            if ($this->common_model->update_saved_data($this->table_name1)) {
                ...
                //$this->common_model->add_log($tDes);
                $this->session->set_flashdata('message_saved', 'Saved successfully.');
                redirect(base_url() . 'adminpanel/academic/processing_unit/gallery/'.$iUnitId);
            } else {
                $this->session->set_flashdata('message_error', 'Save fail!');
                redirect(base_url() . 'adminpanel/academic/processing_unit/gallery/'.$iUnitId);
            }
