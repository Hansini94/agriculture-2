...
class Common_model extends CI_Model {
    ...
    public function add_log($tDes, $title = '') {
        ...
        $logData = array(
         ...
         $this->db->insert('tbl_livestock_logs', $logData);
    ...
    public function save_data($tbl_livestock_name) {
		$fields = $this->db->list_fields($tbl_livestock_name);
        ...
        $query = $this->db->insert($tbl_livestock_name, $data);
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function update_saved_data($tbl_livestock_name) {
        $fields = $this->db->list_fields($tbl_livestock_name);
        ...
        $query = $this->db->update($tbl_livestock_name, $data, "id = $id");
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function update_password_data_useless($tbl_livestock_name) {
        ...
        if ($oldpw == $olpdbpw) {
            ...
            $query = $this->db->update($tbl_livestock_name, $data, "id = $uUserID");
            if ($query) {
                return true;
            } else {
                return false;
    ...
    public function update_password_data($tbl_livestock_name) {
    ...
    function get_edit_data($userID, $tbl_livestock_name) {
        $result = $this->db->get_where($tbl_livestock_name, array('id' => $userID));
    ...
    function chge_status($userID, $tbl_livestock_name) {
		//echo "hh";exit();
        $this->db->select('cEnable');
        $this->db->where('id', $userID);
        $query = $this->db->get($tbl_livestock_name);
        ...
        $this->db->update($tbl_livestock_name, $arrData, "id = $userID");
		//echo $this->db->last_query(); exit();
    }

    function del_records($userID, $tbl_livestock_name) {
        $this->db->where('id', $userID);
        $this->db->delete($tbl_livestock_name);
    }

    function get_sector_detail($sec_id) {
        $this->db->select('vCompanysector');
        $this->db->where('id', $sec_id);
        $query = $this->db->get('tbl_livestock_company_sector');
    ...
    function get_page_detail($page_id) {
        if ($this->session->userdata('is_logged_inbackendsession') != "1") {
            ...
            $query_string = "SELECT
            tbl_livestock_privilage.vPrivilages 
            FROM
            tbl_livestock_privilage
            INNER JOIN tbl_livestock_dyn_menu ON tbl_livestock_privilage.iFormID = tbl_livestock_dyn_menu.id
            WHERE
            tbl_livestock_privilage.iUserTypeID = $iUserType AND
            tbl_livestock_dyn_menu.id = $page_id";
    ...
    function get_last_serial($tbl_livestock_name, $field_name) {
        $query = "select TOP 1 $field_name from $tbl_livestock_name order by id DESC";
